                               ...
                               bool zero_first)
{
    ...
    double weight0 = (bunch.get_real_num() / bunch.get_total_num()) *
                     bunch.get_reference_particle().get_charge() *
                     pconstants::e / (h[0] * h[1] * h[2]);
    int ix, iy, iz;
    ...
    if (rho_grid.get_domain().is_periodic()) {
        for (int n = 0; n < bunch.get_local_num(); ++n) {
            ...
            rho_grid.get_domain().get_leftmost_indices_offsets(
                parts[n][4], parts[n][2], parts[n][0], iz, iy, ix, offz, offy,
                offx);
            for (int i = 0; i < 2; ++i) {
                for (int j = 0; j < 2; ++j) {
                    for (int k = 0; k < 2; ++k) {
...
deposit_charge_rectangular_xyz(Rectangular_grid& rho_grid, Bunch const& bunch,
                               bool zero_first)
{
    // the particles close (i.e at a distance smaller than cell_size/2) to the
    // grid edges are not deposited
                ...
                for (int i = 0; i < 2; ++i) {
                    for (int j = 0; j < 2; ++j) {
                        ...
                        for (int k = 0; k < 2; ++k) {
                            ...
                            cellz = (cellz % period) * (cellz >= 0) +
                                    (period - 1 - ((-cellz - 1) % period)) *
                                        (cellz < 0);
                            //                             if (cellz >= 0) {
                            //                                 cellz = cellz %
                            //                                 period;
    ...
    //  rho_grid.set_normalization(total_charge_per_cell_vol*(h[0] * h[1] *
    //  h[2]));
}
                              ...
                              bool zero_first)
{
    ...
    for (int n = 0; n < bunch.get_local_num(); ++n) {
        ...
        rho_grid.get_domain().get_leftmost_indices_offsets(
            parts[n][0], parts[n][2], parts[n][4], ix, iy, iz, offx, offy,
            offz);
        for (int i = 0; i < 2; ++i) {
            for (int j = 0; j < 2; ++j) {
                int cellx = ix + i;
...
deposit_charge_rectangular_2d(Rectangular_grid& rho_grid,
                              Raw_MArray2d& particle_bin, Bunch const& bunch,
                              bool zero_first)
{
    MArray2dc_ref rho_2dc(rho_grid.get_grid_points_2dc());
    MArray1d_ref rho_1d(rho_grid.get_grid_points_1d());

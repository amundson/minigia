...
private:
    ...
    Commxx& operator=(Commxx const& commxx) { return *this; }

public:
    /// Construct a Commxx object using MPI_COMM_WORLD
    Commxx();

    /// Construct a Commxx object, optionally creating separate communicators on
    /// each unique host for communication avoidance
    Commxx(bool per_host);

    /// Construct a Commxx object based on the parent communicator, optionally
    /// creating separate communicators on each unique host for communication
    /// avoidance
    ...
    Commxx_sptr get_parent_sptr() const;
    ...
    /// MPI_Barrier on the MPI_comm
    void barrier() const { MPI_Barrier(comm); }

    ~Commxx();
};

Commxx_sptr make_optimal_spc_comm(Commxx_sptr parent_sptr, int optimal_number,
                                  bool equally_spread = false);

...
Rectangular_grid::Rectangular_grid(std::vector<double> const& physical_size,
                                   ...
                                   bool periodic_z, storage3d storage)
    : normalization(1.0)
    , storage(storage)
{
    ...
    grid_points_2dc_sptr = boost::shared_ptr<Raw_MArray2dc>(
        new Raw_MArray2dc(boost::extents[grid_shape[0]][grid_shape[1]]));
    grid_points_1d_sptr = boost::shared_ptr<Raw_MArray1d>(
        new Raw_MArray1d(boost::extents[grid_shape[2]]));
}
    ...
    , storage(storage)
{
    ...
    std::vector<int> grid_shape(rectangular_grid_domain_sptr->get_grid_shape());
    grid_points_sptr = boost::shared_ptr<Raw_MArray3d>(new Raw_MArray3d(
        boost::extents[grid_shape[0]][grid_shape[1]][grid_shape[2]], storage));
    grid_points_2dc_sptr = boost::shared_ptr<Raw_MArray2dc>(
        new Raw_MArray2dc(boost::extents[grid_shape[0]][grid_shape[1]]));

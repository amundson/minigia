    ...
    , comm_sptr(comm_sptr)
{
    if (comm_sptr->get_size() / 2 > shape[0] / 2) {
        throw std::runtime_error("Distributed_fft3d: (number of processors)/2 "
                                 "must be <= shape[0]/2");
    }
    ...
    ptrdiff_t fftw_local_size =
        fftw_mpi_local_size_3d(shape[0], shape[1], shape[2], comm_sptr->get(),
    ...
    // GREAT BIG HACK. fftw sometimes crashes when memory_fudge_factor = 1. The
    // test
    // program test_distributed_fft3d_mpi fails when memory_fudge_factor = 1 and
    // numproc = 3. This is either a bug in fftw3, or a failing of the
    // documentation.
    ...
    inv_plan = fftw_mpi_plan_dft_c2r_3d(shape[0], shape[1], shape[2], workspace,
                                        data, comm_sptr->get(), planner_flags);
    lower = local_x_start;
...
Distributed_fft3d::calculate_uppers_lengths()
{
    if (uppers.empty()) {
        ...
        if (size == 1) {
            ...
            MPI_Allgather((void*)(&upper), 1, MPI_INT, (void*)(&uppers[0]), 1,
                          MPI_INT, comm_sptr->get());
        }
...
Distributed_fft3d::transform(MArray3d_ref& in, MArray3dc_ref& out)
{
    if (have_local_data) {
        if (in.index_bases()[0] > lower) {
            throw std::runtime_error("Distributed_fft3d::transform found an "
                                     "incompatible first index offset in input "
                                     "array");
        }
        if (static_cast<int>(in.index_bases()[0] + in.shape()[0]) < upper) {
            throw std::runtime_error("Distributed_fft3d::transform found an "
                                     "incompatible first dimension of input "
                                     "array");
        }
        if (static_cast<int>(in.shape()[1]) != get_padded_shape_real()[1]) {
            throw std::runtime_error("Distributed_fft3d::transform found an "
                                     "incompatible second dimension of input "
                                     "array");
        }
        if (static_cast<int>(in.shape()[2]) != get_padded_shape_real()[2]) {
            throw std::runtime_error("Distributed_fft3d::transform found an "
                                     "incompatible third dimension of input "
                                     "array");
        }
        if (static_cast<int>(out.index_bases()[0]) > lower) {
            throw std::runtime_error("Distributed_fft3d::transform found an "
                                     "incompatible first index offset in "
                                     "output array");
        }
        if (static_cast<int>(out.index_bases()[0] + out.shape()[0]) < upper) {
            throw std::runtime_error("Distributed_fft3d::transform found an "
                                     "incompatible first dimension of output "
                                     "array");
        }
        if (static_cast<int>(out.shape()[1]) != get_padded_shape_complex()[1]) {
            throw std::runtime_error("Distributed_fft3d::transform found an "
                                     "incompatible second dimension of output "
                                     "array");
        }
        if (static_cast<int>(out.shape()[2]) != get_padded_shape_complex()[2]) {
            throw std::runtime_error("Distributed_fft3d::transform found an "
                                     "incompatible third dimension of output "
                                     "array");
        }
    }
#ifdef USE_FFTW2
    ...
    fftw_execute(plan);
    if (have_local_data) {
        memcpy((void*)multi_array_offset(out, lower, 0, 0), (void*)(workspace),
               local_size_complex * sizeof(std::complex<double>));
    }
#endif // USE_FFTW2
}
...
Distributed_fft3d::inv_transform(MArray3dc_ref& in, MArray3d_ref& out)
{
    if (have_local_data) {
        if (in.index_bases()[0] > lower) {
            throw std::runtime_error("Distributed_fft3d::inv_transform found "
                                     "an incompatible first index offset in "
                                     "input array");
        }
        if (static_cast<int>(in.index_bases()[0] + in.shape()[0]) < upper) {
            throw std::runtime_error("Distributed_fft3d::inv_transform found "
                                     "an incompatible first dimension of input "
                                     "array");
        }
        if (static_cast<int>(in.shape()[1]) != get_padded_shape_complex()[1]) {
            throw std::runtime_error("Distributed_fft3d::inv_transform found "
                                     "an incompatible second dimension of "
                                     "input array");
        }
        if (static_cast<int>(in.shape()[2]) != get_padded_shape_complex()[2]) {
            throw std::runtime_error("Distributed_fft3d::inv_transform found "
                                     "an incompatible third dimension of input "
                                     "array");
        }
        if (static_cast<int>(out.index_bases()[0]) > lower) {
            throw std::runtime_error("Distributed_fft3d::inv_transform found "
        ...
        if (static_cast<int>(out.index_bases()[0] + out.shape()[0]) < upper) {
            throw std::runtime_error("Distributed_fft3d::inv_transform found "
                                     "an incompatible first dimension of "
                                     "output array");
        }
        ...
        if (static_cast<int>(out.shape()[2]) != get_padded_shape_real()[2]) {
            throw std::runtime_error("Distributed_fft3d::inv_transform found "
                                     "an incompatible third dimension of "
                                     "output array");
        }
    }
#ifdef USE_FFTW2

...
void
Distributed_rectangular_grid::construct_hockney(
    int lower, int upper, std::vector<int> const& array_shape)
{
    ...
    grid_points_sptr = boost::shared_ptr<Raw_MArray3d>(
        new Raw_MArray3d(boost::extents[extent_range(lower_guard, upper_guard)]
                                       [array_shape[1]][array_shape[2]]));
    grid_points_2dc_sptr = boost::shared_ptr<Raw_MArray2dc>(
        new Raw_MArray2dc(boost::extents[extent_range(lower_guard, upper_guard)]
                                        [array_shape[1]]));
    grid_points_1d_sptr = boost::shared_ptr<Raw_MArray1d>(
        new Raw_MArray1d(boost::extents[array_shape[2]]));
    normalization = 1.0;
}

void
Distributed_rectangular_grid::construct_rectangular(
    int lower, int upper, std::vector<int> const& array_shape)
{
    std::vector<int> grid_shape(domain_sptr->get_grid_shape());
    this->lower = lower;
    ...
    grid_points_sptr = boost::shared_ptr<Raw_MArray3d>(new Raw_MArray3d(
        boost::extents[array_shape[0]][array_shape[1]][array_shape[2]]));
...
Distributed_rectangular_grid::Distributed_rectangular_grid(
    ...
    std::vector<int> const& grid_shape, bool periodic, int lower, int upper,
    Commxx_sptr comm_sptr, std::string const solver)
    : uppers(0)
    , lengths(0)
    , comm_sptr(comm_sptr)
{
...
Distributed_rectangular_grid::fill_guards()
{
        ...
        if (domain_sptr->is_periodic()) {
            ...
            for (int j = 0; j < max1; ++j) {
                for (int k = 0; k < max2; ++k) {
                    (grid_points_sptr->m)[-1][j][k] =
                        (grid_points_sptr->m)[max0 - 1][j][k];
                }
            }
            for (int j = 0; j < max1; ++j) {
                for (int k = 0; k < max2; ++k) {
    ...
    // send to the right
    recv_buffer =
        (void*)multi_array_offset(grid_points_sptr->m, lower_guard, 0, 0);
    send_buffer =
        (void*)multi_array_offset(grid_points_sptr->m, upper - 1, 0, 0);
    size_t message_size =
        grid_points_sptr->m.shape()[1] * grid_points_sptr->m.shape()[2];
    int sender = rank - 1;
    bool send = true;
    int receiver = rank + 1;
    bool receive = true;
    ...
    // send to the left
    recv_buffer =
        (void*)multi_array_offset(grid_points_sptr->m, upper_guard - 1, 0, 0);

...
public:
    Rectangular_grid(std::vector<double> const& physical_size,
                     std::vector<double> const& physical_offset,
                     std::vector<int> const& grid_shape, bool periodic_z,
                     storage3d storage = boost::c_storage_order());
    Rectangular_grid(Rectangular_grid_domain_sptr rectangular_grid_domain_sptr,
                     storage3d storage = boost::c_storage_order());
    Rectangular_grid_domain const& get_domain() const { return *domain_sptr; }
    Rectangular_grid_domain& get_domain() { return *domain_sptr; }
    Rectangular_grid_domain_sptr get_domain_sptr() const;
    Rectangular_grid_domain_sptr get_domain_sptr();
    MArray3d_ref const& get_grid_points() const;
    MArray3d_ref& get_grid_points();
    MArray2dc_ref const& get_grid_points_2dc() const;
    MArray2dc_ref& get_grid_points_2dc();
    MArray1d_ref const& get_grid_points_1d() const;
    MArray1d_ref& get_grid_points_1d();
    void set_normalization(double val);
    double get_normalization() const;
    storage3d get_storage() const;
    //
    ...
    //
    // Tested for the 2D case only, where coordinate X is X and Y is Y. iz = 0,
    // for this case.
    // Other cases needs work most likely. But I am always confused on the
    // convention in
    ...
    inline double get_interpolated_coord(double x, double y, double z) const
    {
        ...
        this->get_domain_sptr()->get_leftmost_indices_offsets(
            x, y, z, ix, iy, iz, offx, offy, offz);
        MArray3d_ref a(this->get_grid_points());
        double val = 0.0;
        if ((get_domain_sptr()->get_grid_shape()[0] > 1) &&
            (get_domain_sptr()->get_grid_shape()[1] > 1) &&
            (get_domain_sptr()->get_grid_shape()[2] > 1)) {
            if ((ix < 0) ||
                (ix >= get_domain_sptr()->get_grid_shape()[0] - 1) ||
                (iy < 0) ||
                (iy >= get_domain_sptr()->get_grid_shape()[1] - 1) ||
                (iz < 0) ||
                (iz >= get_domain_sptr()->get_grid_shape()[2] - 1)) {
                val = 0.0;
            } else {
...
typedef boost::shared_ptr<Rectangular_grid>
    Rectangular_grid_sptr; // syndoc:include
